...
bool CMesh::ReadXML(CXML_Rip *pXML) {
    ...
    if (!pXML->FindLoadElement("DrawSmooth", &DrawSmooth))
        DrawSmooth = false;

    CVertex tmp;
    if (pXML->FindElement("Vertices")) {
        while (pXML->FindElement("Vertex")) {
            if (!pXML->FindLoadElement("Vx", &tmp.v.x))
                tmp.v.x = 0.0;
            if (!pXML->FindLoadElement("Vy", &tmp.v.y))
                tmp.v.y = 0.0;
            if (!pXML->FindLoadElement("Vz", &tmp.v.z))
                tmp.v.z = 0.0;
            if (!pXML->FindLoadElement("Nx", &tmp.n.x))
                tmp.n.x = 0.0;
            if (!pXML->FindLoadElement("Ny", &tmp.n.y))
                tmp.n.y = 0.0;
            if (!pXML->FindLoadElement("Nz", &tmp.n.z))
                tmp.n.z = 0.0;
            if (!pXML->FindLoadElement("R", &tmp.VColor.r))
                tmp.VColor.r = 1.0;
            if (!pXML->FindLoadElement("G", &tmp.VColor.g))
                tmp.VColor.g = 1.0;
            if (!pXML->FindLoadElement("B", &tmp.VColor.b))
                tmp.VColor.b = 1.0;
            if (!pXML->FindLoadElement("A", &tmp.VColor.a))
                tmp.VColor.a = 1.0;
            if (!pXML->FindLoadElement("DOx", &tmp.DrawOffset.x))
                tmp.DrawOffset.x = 0.0;
            if (!pXML->FindLoadElement("DOy", &tmp.DrawOffset.y))
                tmp.DrawOffset.y = 0.0;
            if (!pXML->FindLoadElement("DOz", &tmp.DrawOffset.z))
                tmp.DrawOffset.z = 0.0;
    ...
    if (pXML->FindElement("Facets")) {
        while (pXML->FindElement("Facet")) {
            if (!pXML->FindLoadElement("V0", &Ftmp.vi[0]))
                Ftmp.vi[0] = 0;
            if (!pXML->FindLoadElement("V1", &Ftmp.vi[1]))
                Ftmp.vi[1] = 0;
            if (!pXML->FindLoadElement("V2", &Ftmp.vi[2]))
                Ftmp.vi[2] = 0;
            if (!pXML->FindLoadElement("Nx", &Ftmp.n.x))
                Ftmp.n.x = 0.0;
            if (!pXML->FindLoadElement("Ny", &Ftmp.n.y))
                Ftmp.n.y = 0.0;
            if (!pXML->FindLoadElement("Nz", &Ftmp.n.z))
                Ftmp.n.z = 0.0;
            if (!pXML->FindLoadElement("R", &Ftmp.FColor.r))
                Ftmp.FColor.r = 1.0;
            if (!pXML->FindLoadElement("G", &Ftmp.FColor.g))
                Ftmp.FColor.g = 1.0;
            if (!pXML->FindLoadElement("B", &Ftmp.FColor.b))
                Ftmp.FColor.b = 1.0;
            if (!pXML->FindLoadElement("A", &Ftmp.FColor.a))
                Ftmp.FColor.a = 1.0;
            if (!pXML->FindLoadElement("Name", &Ftmp.Name))
                Ftmp.Name = -1;
    ...
    if (pXML->FindElement("Lines")) {
        while (pXML->FindElement("Line")) {
            if (!pXML->FindLoadElement("V0", &Ltmp.vi[0]))
                Ltmp.vi[0] = 0;
            if (!pXML->FindLoadElement("V1", &Ltmp.vi[1]))
                Ltmp.vi[1] = 0;
...
bool CMesh::LoadSTL(std::string filename) {
    ...
    ret_code = fread(&facenum, sizeof(int), 1, fp);
    if (ret_code != 1) {
        printf("ERROR: ret_code %ld.\n", ret_code);
    }
    int expected_file_size = STL_LABEL_SIZE + 4 + (sizeof(short) + 12 * sizeof(float)) * facenum;
    if (file_size == expected_file_size)
        binary = true;
    unsigned char tmpbuf[128];
    ret_code = fread(tmpbuf, sizeof(tmpbuf), 1, fp);
    if (ret_code != 1) {
        printf("ERROR: ret_code %ld.\n", ret_code);
    }
    ...
    bool RetVal;
    if (binary)
        RetVal = LoadBinarySTL(filename);
    else
        RetVal = LoadAsciiSTL(filename);
...
bool CMesh::LoadBinarySTL(std::string filename) {
    ...
    for (int i = 0; i < facenum; ++i) {
        ret_code = fread(&N, 3 * sizeof(float), 1, fp); // We end up throwing this out and recalculating because... we don't trust it!!!
        if (ret_code != 1) {
            printf("ERROR: ret_code %ld.\n", ret_code);
        }
        ret_code = fread(&P, 3 * sizeof(float), 3, fp);
        if (ret_code != 3) {
            printf("ERROR: ret_code %ld.\n", ret_code);
        }
        ret_code = fread(&attr, sizeof(short), 1, fp);
        if (ret_code != 1) {
            printf("ERROR: ret_code %ld.\n", ret_code);
        }
...
bool CMesh::LoadAsciiSTL(std::string filename) {
    ...
    while (!feof(fp)) {
        ret = fscanf(fp, "%*s %*s %f %f %f\n", &N[0], &N[1],
                     &N[2]); // --> "facet normal 0 0 0" (We throw this out and recalculate based on vertices)
        if (ret != 3) {
            // we could be in the case of a multiple solid object, where after a endfaced instead of another facet we have to skip two
            // lines:
            //     endloop
            //	 endfacet
...
bool CMesh::SaveSTL(std::string filename, bool Binary, bool SaveDeformed) const { // writes ascii stl file...
...
#ifdef WIN32
    if (Binary)
        fopen_s(&fp, filename.c_str(), "wb"); // secure version. preferred on windows platforms...
    else
    ...
    if (Binary) {
        ...
        for (int i = 0; i < NumFaces; i++) {
            ...
            for (int k = 0; k < 3; k++) {
                fwrite(&P[3 * k], 3, sizeof(float), fp);
            }
        ...
        for (int i = 0; i < NumFaces; i++) {
            ...
            for (int k = 0; k < 3; k++) {
                fprintf(fp, "      vertex  %13e %13e %13e\n", Vertices[Facets[i].vi[k]].v.x, Vertices[Facets[i].vi[k]].v.y,
                        Vertices[Facets[i].vi[k]].v.z);
            }
            fprintf(fp, "    endloop\n");
            fprintf(fp, "  endfacet\n");
        }
    ...
    if (bShaded) {
        for (int i = 0; i < (int)Facets.size(); i++) {
            ...
            if (!DrawSmooth) { // if setting things per triangle, can do normal and color here...
                ...
                if (!bIngoreColors)
                    glColor3d(Facets[i].FColor.r, Facets[i].FColor.g, Facets[i].FColor.b);
            }
            for (int j = 0; j < 3; j++) {
...
{
    for (int i = 0; i < (int)Facets.size(); i++) {
        Facets[i].n = ((Vertices[Facets[i].vi[1]].OffPos() - Vertices[Facets[i].vi[0]].OffPos())
                           .Cross(Vertices[Facets[i].vi[2]].OffPos() - Vertices[Facets[i].vi[0]].OffPos()))
                          .Normalized();
    }
}
...
void CMesh::AddFacet(const Vec3D<> &v1, const Vec3D<> &v2, const Vec3D<> &v3,
                     bool QuickAdd) // adds a facet, checks vertex list for existing vertices...
{
    AddFacet(v1, v2, v3, CColor(0.5, 0.5, 0.5, 1.0), CColor(0.5, 0.5, 0.5, 1.0), CColor(0.5, 0.5, 0.5, 1.0));
}

//---------------------------------------------------------------------------
void CMesh::AddFacet(const Vec3D<> &v1, const Vec3D<> &v2, const Vec3D<> &v3, const CColor &Col1, const CColor &Col2, const CColor &Col3,
                     bool QuickAdd) // adds a facet... with color info
//---------------------------------------------------------------------------
{
    ...
    for (int j = 0; j < 3; j++) { // each point in this facet
        ...
        if (!QuickAdd) {
            for (int k = Vertices.size() - 1; k >= 0; k--) { // DO THIS BACKWARDS!!!! (more likely to have just added one next to us...)
                if (abs(Points[j].x - Vertices[k].v.x) < WeldThresh && abs(Points[j].y - Vertices[k].v.y) < WeldThresh &&
                    abs(Points[j].z - Vertices[k].v.z) < WeldThresh) { // if points are identical...
                    FoundIndex[j] = k;
    ...
    Facets.push_back(
        CFacet(FoundIndex[0], FoundIndex[1], FoundIndex[2])); // TODO... select whether to create new object or add to existing...
    MeshChanged();
}
...
//---------------------------------------------------------------------------
{
    ...
    if (count % 2 == 1)
        return true; // if we've passed an off number of facets...
    else
        return false;
}
...
{ // returns the number of intersections, stored in pIntersections. pToCheck is a vector of facet indices that are in this Z plane...
    ...
    for (int i = 0; i < NumtoCheck; i++) { // for each facet we wish to check...
        ...
        if (IntersectXRay(&Facets[pToCheck[i]], y, z, p, pu, pv)) { // if it intersects
            if (InsideTri(p, Vertices[Facets[pToCheck[i]].vi[0]].v, Vertices[Facets[pToCheck[i]].vi[1]].v,
                          Vertices[Facets[pToCheck[i]].vi[2]].v)) {
                pIntersections[NumFound++] = p.x; //(1.0 - pu - pv)*Vertices[Facets[pToCheck[i]].vi[0]].v.x +
                                                  //pu*Vertices[Facets[pToCheck[i]].vi[1]].v.x + pv*Vertices[Facets[pToCheck[i]].vi[2]].v.x;
            }
        }
    }
...
{ // Compute determinant of 3x3 matrix v0,v1,v2
    ...
    return
        ...
        v0.x * (v1.y * v2.z - v1.z * v2.y) + v0.y * (v1.z * v2.x - v1.x * v2.z) + v0.z * (v1.x * v2.y - v1.y * v2.x);
}
...
void CMesh::WeldClose(float Distance) {
    ...
    for (int i = 0; i < (int)Facets.size(); i++) { // look through facets so we don't have to do exhaustive On2 search of all vertex combos
        for (int j = 0; j < 3; j++) {              // look at all three combinations of vertices...
            ...
            while (np != Vi1) {
                np = Vi1;
                Vi1 = ConsolidateMap[Vi1];
            } // iterates NewMap to get the final value...

            int Vi2 = Facets[i].vi[(j + 1) % 3];
            np = -1;
            while (np != Vi2) {
                np = Vi2;
                Vi2 = ConsolidateMap[Vi2];
            } // iterates NewMap to get the final value...

            if (Vi1 != Vi2 &&
                (Vertices[Vi1].v - Vertices[Vi2].v).Length() < Distance) { // if they are close enough but not already the same...
                Vertices[Vi1].v = (Vertices[Vi1].v * NumVertHere[Vi1] + Vertices[Vi2].v * NumVertHere[Vi2]) /
                                  (NumVertHere[Vi1] + NumVertHere[Vi2]); // Vertex 1 is the weighted average
    ...
    for (int i = 0; i < (int)Facets.size(); i++) { // look through facets so we don't have to do exhaustive On2 search of all vertex combos
        ...
        if (!(Facets[i].vi[0] == Facets[i].vi[1] || Facets[i].vi[0] == Facets[i].vi[2] ||
              Facets[i].vi[2] == Facets[i].vi[1])) // if there aren't any the same...
...
void CMesh::FillTriLayer(vfloat z) // fills in TriHeight with all triangles that bridge this plane
{
    ...
    for (FIter = Facets.begin(); FIter != Facets.end(); FIter++) {
        ...
        for (int n = 0; n < 3; n++) {
            if (Vertices[FIter->vi[n]].v.z > z)
                IsBelow = false;
            if (Vertices[FIter->vi[n]].v.z < z)
                IsAbove = false;
        }
        if (!IsAbove && !IsBelow)
            TriLayer.push_back(m); // if this facet is not fully above or fully below our ZPlane
