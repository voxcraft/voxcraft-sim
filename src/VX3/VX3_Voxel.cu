...
VX3_Voxel::VX3_Voxel(CVX_Voxel *p, VX3_VoxelyzeKernel *k)
    : matid(p->matid), ix(p->ix), iy(p->iy), iz(p->iz), pos(p->pos), linMom(p->linMom), orient(p->orient), angMom(p->angMom),
      ...
      shiftCiliaForce(p->shiftCiliaForce),detachTime(p->detachTime), activationTimeConstant(p->activationTimeConstant), 
      recoveryTimeConstant(p->recoveryTimeConstant), downregulationConstant(p->downregulationConstant) {
    _voxel = p;
