//
// Created by Sida Liu
//  This is a device version of CVX_Voxel class in VX2.
//
...
public:
    ...
    __device__ VX3_Voxel *adjacentVoxel(linkDirection direction) const; //!< Returns a pointer to the voxel in the specified direction if one exists, or NULL otherwise. @param[in] direction
    ...
    __device__ VX3_MaterialVoxel *material() { return mat; } //!< Returns the linked material object containing the physical properties of this voxel.
    ...
    __device__ void timeStep(double dt, double currentTime,
    ...
    // physical location
    __device__ VX3_Vec3D<double> position() const { return pos; } //!< Returns the center position of this voxel in meters (GCS). This is the origin of the local coordinate system (LCS).
    ...
    __device__ VX3_Vec3D<double> displacement() const { return (pos - originalPosition()); } //!< Returns the 3D displacement of this voxel from its original location in meters (GCS)/
    ...
    __device__ VX3_Vec3D<float> cornerPosition(voxelCorner corner) const;                //!< Returns the deformed location of the voxel corner in the specified corner in the global coordinate
                                                                                         //!< system (GCS). Essentially cornerOffset() with the voxel's current global position/rotation applied.
    __device__ VX3_Vec3D<float> cornerOffset(voxelCorner corner) const;                  //!< Returns the deformed location of the voxel corner in the specified corner in the local voxel coordinate system (LCS).
                                                                                         //!< Used to draw the deformed voxel in the correct position relative to the position().
    __device__ bool isInterior() const { return (boolStates & SURFACE) ? true : false; } //!< Returns true if the voxel is surrounded by other voxels on its 6 coordinate faces. Returns false if 1 or more faces are exposed.
    ...
    __device__ float orientationAngle() const { return (float)orient.Angle(); } //!< Use with orientationAxis() to get the orientation of this voxel in angle/axis form. Returns the angle in radians.
    ...
    __device__ float displacementMagnitude() const { return (float)displacement().Length(); }                                                                //!< Returns the distance (magnitude of displacement) this voxel has moved from its initial nominal position. (GCS)
    __device__ float angularDisplacementMagnitude() const { return (float)orient.Angle(); }                                                                  //!< Returns the angle (magnitude of angular displacement) this voxel has rotated from its initial nominal orientation. (GCS)
    __device__ VX3_Vec3D<double> velocity() const { return linMom * mat->_massInverse; }                                                                     //!< Returns the 3D velocity of this voxel in m/s (GCS)
    __device__ float velocityMagnitude() const { return (float)(linMom.Length() * mat->_massInverse); }                                                      //!< Returns the velocity of this voxel in m/s.
    __device__ VX3_Vec3D<double> angularVelocity() const { return angMom * mat->_momentInertiaInverse; }                                                     //!< Returns the 3D angular velocity of this voxel in rad/s (GCS)
    __device__ float angularVelocityMagnitude() const { return (float)(angMom.Length() * mat->_momentInertiaInverse); }                                      //!< Returns the angular velocity of this voxel in rad/s.
    __device__ float kineticEnergy() const { return (float)(0.5 * (mat->_massInverse * linMom.Length2() + mat->_momentInertiaInverse * angMom.Length2())); } //!< Returms the kinetic energy of this voxel in Joules.
    ...
    // material state
    __device__ bool isYielded() const; //!< Returns true if the stress in this voxel has ever exceeded the yield stress. Technically, this returns true if
                                       //!< any of the connected links have yielded since the stress state of the voxel is never expressly calculated.
    __device__ bool isFailed() const;  //!< Returns true if the stress in this voxel has ever exceeded the failure stress. Technically, this returns true if
    ...
    __device__ void setTemperature(float temperature); //!< Specifies the temperature for this voxel. This adds (or subtracts) the correct amount of thermal
    ...
    __device__ void haltMotion() { linMom = angMom = VX3_Vec3D<>(0, 0, 0); } //!< Halts all momentum of this block. Unless fixed the voxel will continue to move in subsequent timesteps.

    __device__ void enableFloor(bool enabled) { enabled ? boolStates |= FLOOR_ENABLED : boolStates &= ~FLOOR_ENABLED; } //!< Enables this voxel interacting with the floor at Z=0. @param[in] enabled Enable interaction
    __host__ __device__ bool isFloorEnabled() const { return boolStates & FLOOR_ENABLED ? true : false; }               //!< Returns true of this voxel will interact with the floor at Z=0.
    ...
    __device__ VX3_Vec3D<double> force();  //!< Calculates and returns the sum of the current forces on this voxel. This would normally only be called internally, but can
                                           //!< be used to query the state of a voxel for visualization or debugging.
    __device__ VX3_Vec3D<double> moment(); //!< Calculates and returns the sum of the current moments on this voxel. This would normally only be called internally, but
                                           //!< can be used to query the state of a voxel for visualization or debugging.

    __device__ float transverseArea(linkAxis axis);      //!< Returns the transverse area of this voxel with respect to the specified axis. This would normally be
    ...
    __device__ float transverseStrainSum(linkAxis axis); //!< Returns the sum of the current strain of this voxel in the two mutually perpindicular axes to
    ...
    __device__ static inline linkDirection toDirection(linkAxis axis, bool positiveDirection) { return (linkDirection)(2 * ((int)axis) + positiveDirection ? 0 : 1); } //!< Returns the link direction of the specified link axis and sign.
    __device__ static inline bool isNegative(linkDirection direction) { return direction % 2 == 1; }                                                                   //!< Returns true if the specified link direction is negative.
    __device__ static inline bool isPositive(linkDirection direction) { return direction % 2 == 0; }                                                                   //!< Returns true if the specified link direction is positive.
    __device__ static inline linkDirection toOpposite(linkDirection direction) { return (linkDirection)(direction - direction % 2 + (direction + 1) % 2); }            //!< Returns the opposite (negated) link direction of the specified direction.

    __device__ void replaceMaterial(VX3_MaterialVoxel *newMaterial); //!< Replaces the material properties of this voxel (but not links) to this new CVX_Material. May
    ...
    __device__ void removeLinkInfo(linkDirection direction); // removes the information about a link connected to this voxel in the specified direction

    __device__ void setFloorStaticFriction(bool active) { active ? boolStates |= FLOOR_STATIC_FRICTION : boolStates &= ~FLOOR_STATIC_FRICTION; }

    __device__ void floorForce(float dt,
    ...
    __device__ VX3_Vec3D<float> strain(bool poissonsStrain) const; // LCS returns voxel strain. if tensionStrain true and no actual tension in that
    ...
    bool middleVoxel = false; // sam
    int numNeigh = 0; // sam
    // VX3_Vec3D<> prevGroupCoM;  // sam
