...
#define DISCARD_ANGLE_RAD                                                      \
    1e-7 // Anything less than this angle can be considered 0
#define SMALL_ANGLE_RAD                                                        \
    1.732e-2 // Angles less than this get small angle approximations. To get:
             // Root solve atan(t)/t-1+MAX_ERROR_PERCENT. From: MAX_ERROR_PERCENT
             // = (t-atan(t))/t
...
*/
template <typename T = double> class VX3_Quat3D {
public:
    ...
    // constructors
    VX3_Quat3D(const Quat3D<T> *QuatIn) : w(1), x(0), y(0), z(0) {
        if (QuatIn) {
            w = QuatIn->w;
            x = QuatIn->x;
            y = QuatIn->y;
    ...
    VX3_Quat3D(const Quat3D<T> QuatIn) {
        w = QuatIn.w;
        x = QuatIn.x;
        y = QuatIn.y;
        z = QuatIn.z;
    }
    __device__ VX3_Quat3D(void)
        : w(1), x(0), y(0), z(0) {
    } //!< Constructor. Initialzes w, x, y, z to zero.
    __device__ VX3_Quat3D(const T dw, const T dx, const T dy, const T dz) {
        w = dw;
        x = dx;
        y = dy;
        z = dz;
    } //!< Constructor with specified individual values.
    __device__ VX3_Quat3D(const VX3_Quat3D &QuatIn) {
        w = QuatIn.w;
        x = QuatIn.x;
        y = QuatIn.y;
        z = QuatIn.z;
    } //!< Copy constructor
    __device__ VX3_Quat3D(const VX3_Vec3D<T> &VecIn) {
    ...
    __device__ VX3_Quat3D(const T angle, const VX3_Vec3D<T> &axis) {
      ...
      //!< rotation axis.
    __device__ VX3_Quat3D(const VX3_Vec3D<T> &RotateFrom,
                          const VX3_Vec3D<T> &RotateTo) {
        T theta = acos(RotateFrom.Dot(RotateTo) /
                       sqrt(RotateFrom.Length2() *
                            RotateTo.Length2())); // angle between vectors. from
                                                  // A.B=|A||B|cos(theta)
        ...
        if (theta > PI - DISCARD_ANGLE_RAD) {
          ...
          // Quat(0,ax,ay,az))
        *this = VX3_Quat3D(
            theta, Axis); // otherwise for the quaternion from angle-axis.
    ...
    template <typename U>
    __device__ VX3_Quat3D<T>(const VX3_Quat3D<U> &QuatIn) {
        w = QuatIn.w;
        x = QuatIn.x;
        y = QuatIn.y;
        z = QuatIn.z;
    } //!< Copy constructor from another template type
    template <typename U> __device__ VX3_Quat3D<T>(const VX3_Vec3D<U> &VecIn) {
        w = 0;
        x = VecIn.x;
        y = VecIn.y;
        z = VecIn.z;
    } //!< Copies x, y, z from the specified vector and sets w to zero.
    template <typename U> __device__ operator VX3_Quat3D<U>() const {
        return VX3_Quat3D<U>(w, x, y, z);
    } //!< overload conversion operator for different template types
    template <typename U>
    __device__ VX3_Quat3D<T> operator=(const VX3_Quat3D<U> &s) {
        w = s.w;
        x = s.x;
        y = s.y;
        z = s.z;
    ...
    template <typename U>
    __device__ const VX3_Quat3D<T> operator+(const VX3_Quat3D<U> &s) {
        return VX3_Quat3D<T>(w + s.w, x + s.x, y + s.y, z + s.z);
    } //!< Addition operator for different template types
    template <typename U>
    __device__ const VX3_Quat3D<T> operator*(const U &f) const {
        return VX3_Quat3D<T>(f * w, f * x, f * y, f * z);
    } //!< Scalar multiplication operator for different template types
    template <typename U>
    __device__ const VX3_Quat3D<T> operator*(const VX3_Quat3D<U> &f) const {
        return VX3_Quat3D(w * f.w - x * f.x - y * f.y - z * f.z,
    ...
    // overload operators
    __device__ VX3_Quat3D &operator=(const VX3_Quat3D &s) {
        w = s.w;
        x = s.x;
        y = s.y;
        z = s.z;
    ...
    __device__ const VX3_Quat3D operator+(const VX3_Quat3D &s) const {
        return VX3_Quat3D(w + s.w, x + s.x, y + s.y, z + s.z);
    } //!< overload additoon
    __device__ const VX3_Quat3D operator-(const VX3_Quat3D &s) const {
        return VX3_Quat3D(w - s.w, x - s.x, y - s.y, z - s.z);
    } //!< overload subtraction
    __device__ const VX3_Quat3D operator*(const T f) const {
        return VX3_Quat3D(w * f, x * f, y * f, z * f);
    } //!< overload scalar multiplication
    __device__ const VX3_Quat3D friend operator*(const T f,
                                                 const VX3_Quat3D v) {
        return VX3_Quat3D(v.w * f, v.x * f, v.y * f, v.z * f);
    } //!< overload scalar multiplication with number first.
    __device__ const VX3_Quat3D operator*(const VX3_Quat3D &f) const {
        return VX3_Quat3D(w * f.w - x * f.x - y * f.y - z * f.z,
                          w * f.x + x * f.w + y * f.z - z * f.y,
                          w * f.y - x * f.z + y * f.w + z * f.x,
                          w * f.z + x * f.y - y * f.x + z * f.w);
    } //!< overload quaternion multiplication.
    __device__ bool operator==(const VX3_Quat3D &s) const {
        return (w == s.w && x == s.x && y == s.y && z == s.z);
    } //!< overload is equal.
    __device__ bool operator!=(const VX3_Quat3D &s) const {
        return (w != s.w || x != s.x || y != s.y || z != s.z);
    } //!< overload is not equal.
    __device__ const VX3_Quat3D &operator+=(const VX3_Quat3D &s) {
        w += s.w;
        x += s.x;
        y += s.y;
        z += s.z;
    ...
    __device__ const VX3_Quat3D Inverse() const {
        T n = w * w + x * x + y * y + z * z;
    ...
    __device__ const VX3_Quat3D Conjugate() const {
    ...
    __device__ const T Angle() const {
        return 2.0 * acos(w > 1 ? 1 : w);
    } //!< Returns the angular rotation of this quaternion in radians.
    __device__ const T AngleDegrees() const {
        return Angle() * 57.29577951308232;
    } //!< Returns the angular rotation of this quaternion in degrees.
    __device__ bool IsNegligibleAngle() const {
    ...
    __device__ VX3_Vec3D<T> Axis() const {
        T squareLength = 1.0 - w * w; // because x*x + y*y + z*z + w*w = 1.0,
                                      // but more susceptible to w noise
        if (squareLength <= 0) {
            return VX3_Vec3D<T>(1, 0, 0);
        } else {
            return VX3_Vec3D<T>(x, y, z) / sqrt(squareLength);
        }
    } //!< Returns the normalized axis of rotation of this quaternion
    __device__ VX3_Vec3D<T> AxisUnNormalized() const {
        return VX3_Vec3D<T>(x, y, z);
    } //!< Returns the un0normalized axis of rotation of this quaternion
    __device__ void AngleAxis(T &angle, VX3_Vec3D<T> &axis) const {
      ...
      //!< form.
    __device__ void AngleAxisUnNormalized(T &angle, VX3_Vec3D<T> &axis) const {
        if (w >= 1.0) {
            angle = 0;
    ...
    __device__ const VX3_Vec3D<T> ToRotationVector() const {
        if (w >= 1.0 || w <= -1.0)
        ...
        if (squareLength < SLTHRESH_ACOS2SQRT)
            return VX3_Vec3D<T>(x, y, z) * 2.0 *
                   sqrt((2 - 2 * w) /
                        squareLength); // acos(w) = sqrt(2*(1-x)) for w close
                                       // to 1. for w=0.001, error is 1.317e-6
        else
            return VX3_Vec3D<T>(x, y, z) * 2.0 * acos(w) / sqrt(squareLength);
    } //!< Returns a rotation vector representing this quaternion rotation.
      //!< Adapted from
      //!< http://www.euclideanspace.com/maths/geometry/rotations/conversions/quaternionToAngle/

    __device__ void FromRotationVector(const VX3_Vec3D<T> &VecIn) {
        VX3_Vec3D<T> theta = VecIn / 2;
        T s, thetaMag2 = theta.Length2();
        if (thetaMag2 * thetaMag2 <
    ...
    __device__ void FromAngleToPosX(
        const VX3_Vec3D<T>
            &RotateFrom) { // highly optimized at the expense of readability
        if (VX3_Vec3D<T>(0, 0, 0) == RotateFrom)
        ...
        if (YoverX < SMALL_ANGLE_RAD && YoverX > -SMALL_ANGLE_RAD &&
            ZoverX < SMALL_ANGLE_RAD &&
            ZoverX > -SMALL_ANGLE_RAD) { // Intercept small angle and zero angle
            x = 0;
            y = 0.5 * ZoverX;
            z = -0.5 * YoverX;
        ...
        VX3_Vec3D<> RotFromNorm = RotateFrom;
        RotFromNorm.NormalizeFast(); // Normalize the input...

        T theta =
            acos(RotFromNorm
                     .x); // because RotFromNorm is normalized, 1,0,0 is
        ...
        const T AxisMagInv = 1.0 / sqrt(RotFromNorm.z * RotFromNorm.z +
                                        RotFromNorm.y * RotFromNorm.y);
        // Here theta is the angle, axis is RotFromNorm.Cross(VX3_Vec3D(1,0,0))
        // = VX3_Vec3D(0, RotFromNorm.z/AxisMagInv, -RotFromNorm.y/AxisMagInv),
      ...
      //!< transform the specified RotateFrom vector to point in the positve X
      //!< direction. Note: function changes this quaternion.  @param[in]
      //!< RotateFrom An arbitrary direction vector. Does not need to be
      //!< normalized.

    __device__ const VX3_Vec3D<T> RotateVec3D(const VX3_Vec3D<T> &f) const {
    ...
    template <typename U>
    __device__ const VX3_Vec3D<U> RotateVec3D(const VX3_Vec3D<U> &f) const {
    ...
    __device__ const VX3_Vec3D<T> RotateVec3DInv(const VX3_Vec3D<T> &f) const {
        T fx = f.x, fy = f.y, fz = f.z;
        T tw = x * fx + y * fy + z * fz;
        T tx = w * fx - y * fz + z * fy;
        T ty = w * fy + x * fz - z * fx;
    ...
    template <typename U>
    __device__ const VX3_Vec3D<U> RotateVec3DInv(const VX3_Vec3D<U> &f) const {
        T fx = f.x, fy = f.y, fz = f.z;
        T tw = x * fx + y * fy + z * fz;
        T tx = w * fx - y * fz + z * fy;
        T ty = w * fy + x * fz - z * fx;
